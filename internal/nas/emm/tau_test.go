...
func TestEncodeTAUAccept_NoGUTI(t *testing.T) {
	...
	// T3412 is a TV IE: IEI 0x5A followed directly by the timer value.
	// The following IE is the TAI list IEI 0x54.
	if len(b) < 6 {
		t.Fatalf("output too short for T3412 and TAI list: %x", b)
	}
	if got, want := b[3:6], []byte{0x5A, 0x21, 0x54}; !bytes.Equal(got, want) {
		t.Fatalf("T3412 TV encoding got %x, want %x", got, want)
	}
...
func TestEncodeTAUAccept_T3412UsesTVFormat(t *testing.T) {
	tai := emm.TAI{
		PLMN: [3]byte{0x13, 0x51, 0x34},
		TAC:  0x0001,
	}
	...
	if !bytes.Contains(b, []byte{0x5A, 0x49, 0x54}) {
		t.Fatalf("TAU Accept missing TV-encoded T3412 followed by TAI list: %x", b)
	}
	if bytes.Contains(b, []byte{0x5A, 0x01, 0x49}) {
		t.Fatalf("TAU Accept incorrectly encoded T3412 as TLV: %x", b)
	}
	...
	if decoded.T3412 == nil || *decoded.T3412 != 0x49 {
		t.Fatalf("decoded T3412 got %v, want 0x49", decoded.T3412)
	}
}
...
func TestEncodeTAUAcceptWithParams_EMMCause(t *testing.T) {
	...
	if decoded.EMMCause == nil || *decoded.EMMCause != cause {
		t.Fatalf("EMM cause got %v, want %#x", decoded.EMMCause, cause)
	}
}

func TestEncodeTAUAcceptWithParams_SMSOnlyCombinedTAU(t *testing.T) {
	tai := emm.TAI{PLMN: [3]byte{0x00, 0xf1, 0x10}, TAC: 1}
	lai := emm.LAI{PLMN: tai.PLMN, LAC: 1}
	additional := uint8(emm.AdditionalUpdateResultSMSOnly)
	b := emm.EncodeTAUAcceptWithParams(emm.TAUAcceptParams{
	...
	if decoded.LAI == nil || decoded.LAI.LAC != lai.LAC || decoded.AdditionalUpdateResult == nil || *decoded.AdditionalUpdateResult != additional {
		t.Fatalf("decoded SMS-only fields got lai=%+v additional=%v", decoded.LAI, decoded.AdditionalUpdateResult)
	}
}
