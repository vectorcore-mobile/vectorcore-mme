...
func DecodeDetachRequest(data []byte) (*DetachRequest, error) {
	if len(data) < 1 {
		return nil, fmt.Errorf("emm: detach request too short")
	}
	dr := &DetachRequest{}
	...
	if len(data) > 1 {
		...
		if 2+mobileIDLen > len(data) {
			return nil, fmt.Errorf("emm: detach request mobile identity truncated: len=%d need=%d", len(data)-2, mobileIDLen)
		}
...
func DetachTypeName(t uint8) string {
	switch t {
	case DetachTypeNormal:
		return "eps-detach"
	case DetachTypeIMSIDetach:
		return "imsi-detach"
	case DetachTypeEPSAndIMSI:
		return "combined-eps-imsi-detach"
	default:
		return "unknown"
	}
}
