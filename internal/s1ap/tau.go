...
func (s *Server) buildTAUAcceptNAS(ue *uecontext.Context, log *zap.Logger, opts tauAcceptOptions) ([]byte, error) {
	...
	tauAcceptPDU := emm.EncodeTAUAcceptWithParams(emm.TAUAcceptParams{
		...
		EPSNetworkFeatureSupport: s.epsNetworkFeatureSupport(ue),
		LAI:                      opts.LAI,
	...
	log.Info("nas: TAU Accept encoded",
		...
		zap.Bool("ims_voice_over_ps_advertised", s.epsNetworkFeatureSupport(ue) != nil),
		...
		zap.String("eps_network_feature_support_hex", fmt.Sprintf("%x", encodeFeatureSupportForLog(s.epsNetworkFeatureSupport(ue)))),
